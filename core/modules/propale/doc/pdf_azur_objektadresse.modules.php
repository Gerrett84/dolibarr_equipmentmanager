...
/**
 *	Class to generate PDF proposal Azur with Objektadresse
 */
class pdf_azur_objektadresse extends ModelePDFPropales
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "azur_objektadresse";
		$this->description = $langs->trans('DocModelAzurDescription').' + Objektadresse';
		...
		if ($showaddress) {
			...
			if (!empty($idaddressshipping) && is_array($idaddressshipping) && count($idaddressshipping) > 0) {
				...
				if ($contactshipping->fetch($idaddressshipping[0]) > 0) {
					...
					if ($contactshipping->address) {
						$carac_objektadresse .= $contactshipping->address."\n";
					}
					$cityline = '';
					if ($contactshipping->zip) {
						$cityline .= $contactshipping->zip;
					}
					if ($contactshipping->town) {
						$cityline .= ($cityline ? ' ' : '').$contactshipping->town;
					}
					if ($cityline) {
