 ...
 *
 * Equipment-Tab on Commande (Auftrag) Card
 * v4.4: Link equipment to orders for automatic transfer to interventions
 */
...
$langs->loadLangs(array('orders', 'equipmentmanager@equipmentmanager'));
...
// Import equipment from linked Propal
if ($action == 'import_from_propal' && $permissiontoadd) {
    $propal_id = GETPOST('propal_id', 'int');
    if ($propal_id > 0) {
        $count = DocumentEquipmentLink::copyLinks($db, $propal_id, 'propal', $object->id, 'commande', $user);
        if ($count > 0) {
...
if ($action == 'link' && $permissiontoadd && $equipment_id > 0 && in_array($link_type, array('maintenance', 'service'))) {
    $link = new DocumentEquipmentLink($db, 'commande');
...
if ($action == 'update_link_type' && $permissiontoadd && $equipment_id > 0) {
    ...
    if (in_array($new_type, array('maintenance', 'service'))) {
        $sql = "UPDATE ".MAIN_DB_PREFIX."equipmentmanager_commande_equipment";
        $sql .= " SET link_type = '".$db->escape($new_type)."'";
        $sql .= " WHERE fk_commande = ".(int)$object->id;
...
$title = $langs->trans('Order').' - '.$langs->trans('Equipment');
llxHeader('', $title);

if ($object->id > 0) {
    $head = commande_prepare_head($object);

    print dol_get_fiche_head($head, 'equipmentmanager', $langs->trans('Order'), -1, 'order');
    ...
    // Check for linked Propal with equipment
    $linkedPropals = array();
    $sql = "SELECT ee.fk_source, p.ref, p.rowid";
    $sql .= " FROM ".MAIN_DB_PREFIX."element_element as ee";
    $sql .= " INNER JOIN ".MAIN_DB_PREFIX."propal as p ON p.rowid = ee.fk_source";
    $sql .= " WHERE ee.fk_target = ".(int)$object->id;
    $sql .= " AND ee.sourcetype = 'propal'";
    $sql .= " AND ee.targettype = 'commande'";

    $resql = $db->query($sql);
    if ($resql) {
        while ($obj = $db->fetch_object($resql)) {
            // Check if propal has equipment linked
            $sqlCheck = "SELECT COUNT(*) as nb FROM ".MAIN_DB_PREFIX."equipmentmanager_propal_equipment WHERE fk_propal = ".(int)$obj->rowid;
    ...
    // Show import from Propal section if there are linked propals with equipment
    if (count($linkedPropals) > 0 && $permissiontoadd) {
        print '<div class="info" style="margin-bottom: 15px;">';
        print '<span class="fa fa-info-circle paddingright"></span>';
        print $langs->trans('PropalHasEquipment').' ';
        foreach ($linkedPropals as $lp) {
            print '<a href="'.DOL_URL_ROOT.'/comm/propal/card.php?id='.$lp->rowid.'">'.$lp->ref.'</a> ';
            print '<a class="button small" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&action=import_from_propal&propal_id='.$lp->rowid.'&token='.newToken().'">';
    ...
    $linkHelper = new DocumentEquipmentLink($db, 'commande');
